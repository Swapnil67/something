                          ...
                          Camera camera)
{
  ...
  const SDL_Rect tile_rect = {
      int(floorf(game_state.mouse_position.x / (float)TILE_SIZE) * TILE_SIZE - camera.pos.x),
      int(floorf(game_state.mouse_position.y / (float)TILE_SIZE) * TILE_SIZE - camera.pos.y),
      TILE_SIZE, TILE_SIZE};
